...
# # SQLALCHEMY_DATABASE_URL= 'postgresql://postgres:Pratyush@localhost:5432/faastapi_test'
...
#     user_data={"email":"hasheduser1@gmail.com",
...
#     #                  models.Post(title="second title",content="second contentf",owner_id=test_user['id']),
#     #                  models.Post(title="3rd title",content="3rd contentf",owner_id=test_user['id'])])
#     session.commit()
...
# Use test database (make sure it exists in CI!)
...
def test_posts(test_user, session):
    ...
    session.commit()
    for post in posts:
        session.refresh(post)  # ensure IDs are loaded
    return posts
